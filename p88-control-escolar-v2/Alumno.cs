...
public override string ToString() =>
    ($"Nombre: {Nombre,-12}, Edad: {Edad}, FechaIng: {FechaIng.ToString("dd/MM/yyyy")}, Becado: {(Becado?"Si":"No")}, ") +
    ($"Califs: {string.Join(",",Califs)}, Antiguedad: {DateTime.Now.Year - FechaIng.Year}, Prom: {Promedio()}, Mensaje:{(Promedio()>=7?"Aprobado":"Reporbado")}");
}